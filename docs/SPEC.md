...
| `Payment` | `userId` → User nullable with set-null on delete, `planId` enum basic/plus/pro, `amountInr` int, `utr` varchar(32), unique except rejected, `studentNote` text nullable, `status` enum pending/approved/rejected/refunded, `reviewNote` text nullable, `reviewedAt` nullable, `reviewedBy` text nullable |
