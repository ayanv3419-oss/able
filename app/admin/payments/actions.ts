...
function requireId(formData: FormData, field: string, label: string): string {
  const value = formData.get(field);

  if (!z.uuid().safeParse(value).success || typeof value !== "string") {
    throw new ChatbotError("bad_request:api", `Missing ${label} id.`);
  }
...
/** Rejecting also blocks the student, per the owner's board. */
...
export async function reviewPaymentAction(
  ...
  try {
    ...
    const paymentId = requireId(formData, "paymentId", "payment");
    ...
    if (decision === "approve") {
      ...
      return { error: "Choose Allow or Reject." };
    }
  } catch (error) {
...
export async function unblockStudentAction(formData: FormData): Promise<void> {
  await requireAdmin();
  const userId = requireId(formData, "userId", "student");
